...
const formSchema = z
  .object({
    ...
    email: z
      ...
      .refine((email) => email.endsWith("@monitor.gov.in"), {
        message: "Email must be from the domain @monitor.gov.in.",
      }),
...
export default function SignUpForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    console.log(values);
    try {
      const response = await fetch("/api/auth/signup/monitor", {
        method: "POST",
        headers: {
      ...
      <Card className="max-w-2xl mx-auto my-8">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold text-blue-900">
            Government Monitor Portal Registration
          </CardTitle>
          <CardDescription>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Monitor Email</FormLabel>
                    <FormControl>
                      <Input
                        placeholder="Monitor email address (@monitor.gov.in)"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
